...
export default function ProfilePage() {
  ...
  const [connectionsView, setConnectionsView] = useState<ConnectionsView>(null);
  ...
  const displayedPosts = postSearchQuery.trim()
    ? posts.filter((p) =>
        (p.record?.text || "")
          .toLowerCase()
          .includes(postSearchQuery.trim().toLowerCase())
      )
    : posts;
  ...
  return (
    <div className="flex min-h-screen w-full bg-kelo-background font-sans text-kelo-text">
        ...
        <div className="px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm">
            <button
              ...
              className="rounded-md text-left transition hover:text-kelo-primary focus:outline-none focus:ring-2 focus:ring-kelo-primary/20"
            >
            ...
            {visibleTabs.map((tab) => (
              <button
                ...
                className={`whitespace-nowrap border-b-2 pb-3 transition-colors ${
                  activeTab === tab
                    ? "border-kelo-primary text-kelo-text"
                    : "border-transparent hover:text-kelo-text"
                }`}
              >
                {tab}
              </button>
            ...
            {lists.map((list) => (
              ...
              >
                ...
                <div className="min-w-0">
                  <h3 className="truncate text-sm font-extrabold">{list.name}</h3>
                  {list.description && (
            ...
            {starterPacks.map((pack) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  {(pack.record?.description || pack.list?.description) && (
                    <p className="mt-1 line-clamp-2 text-xs text-kelo-muted">
                      {pack.record?.description || pack.list?.description}
                    </p>
                  )}
      ...
      <ProfileConnectionsModal
        actor={profile?.did || targetHandle}
        type={connectionsView || "followers"}
        open={connectionsView !== null}
