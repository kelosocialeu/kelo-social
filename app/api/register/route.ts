...
async function verifyCaptcha(token: string): Promise<boolean> {
  ...
  const body = new URLSearchParams({ secret: hcaptchaSecret, response: token });
...
async function getPdsSignupRequirements(): Promise<PdsSignupRequirements> {
  ...
  if (!response.ok) {
    throw new Error(`Impossible de vérifier la configuration du PDS Kelo Social (${response.status}).`);
  }
...
async function createInviteCode(): Promise<string> {
  ...
  if (!adminPassword) {
    throw new Error("Le PDS exige un code d'invitation mais PDS_ADMIN_PASSWORD n'est pas configuré sur Kelo Social.");
  }
  const response = await fetch(
  ...
  if (!response.ok) {
    ...
    throw new Error(`Impossible de générer le code d'invitation sur le PDS (${response.status}).`);
  }
...
function mapRegistrationError(error: unknown): { message: string; status: number } {
  ...
  const message = candidate?.message || (error instanceof Error ? error.message : "Erreur lors de la création du compte.");

  if (/handle.*taken|already.*handle/i.test(message)) return { message: "Ce nom d'utilisateur est déjà utilisé.", status: 409 };
  if (/email.*taken|already.*email/i.test(message)) return { message: "Cette adresse e-mail est déjà utilisée.", status: 409 };
  if (/verification|captcha|invalidtoken|expiredtoken/i.test(message)) {
    return { message: "La vérification anti-robot a expiré ou a échoué. Veuillez refaire le captcha.", status: 400 };
  }
  if (/password/i.test(message)) return { message, status: 400 };
  return {
    message,
    status: typeof candidate?.status === "number" && candidate.status >= 400 && candidate.status < 500 ? candidate.status : 500,
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    if (requirements.verificationRequired && !verificationCode) {
      return NextResponse.json(
        { error: "VALIDATION_PDS_REQUISE", verificationRequired: true },
        { status: 428 }
      );
    }

    if (!requirements.verificationRequired && !(await verifyCaptcha(hcaptchaToken))) {
      return NextResponse.json({ error: "La vérification anti-robot a échoué. Veuillez réessayer." }, { status: 400 });
    }

    const inviteCode = requirements.inviteCodeRequired ? await createInviteCode() : undefined;
    ...
    return NextResponse.json({ success: true, handle: fullHandle, pdsUrl: KELO_PDS_URL });
  } catch (error: unknown) {
