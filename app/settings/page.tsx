...
const SECTION_KEYS:Record<SettingsSection,[string,string]>={
  account:["settings.account","Compte et sécurité"],
  identity:["settings.identity","Identité et domaine"],
  moderation:["settings.moderation","Modération"],
  privacy:["settings.privacy","Confidentialité"],
  appearance:["settings.display","Affichage"],
  language:["settings.languageContent","Langues et centres d’intérêt"],
  notifications:["settings.notifications","Notifications et flux"],
  messaging:["settings.messaging","Messagerie"],
  legal:["settings.legal","Informations juridiques"],
};
    ...
    <main className="min-h-screen min-w-0 flex-1 bg-white pb-24 md:border-x md:border-kelo-border md:pb-0">
      <header className="sticky top-[calc(56px+env(safe-area-inset-top))] z-20 border-b border-kelo-border bg-white/95 px-4 py-3.5 backdrop-blur md:top-0 sm:px-5 md:py-4">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-4">
          <div className="min-w-0"><h1 className="text-xl font-extrabold text-kelo-text">{t("settings.title","Paramètres")}</h1>{handle&&<p className="mt-0.5 truncate text-sm text-kelo-muted">@{handle}</p>}</div>
          <button type="button" onClick={handleLogout} className="flex min-h-10 shrink-0 items-center gap-2 rounded-full border border-kelo-border px-3 text-sm font-bold text-kelo-muted transition hover:bg-red-50 hover:text-red-600 lg:hidden"><LogOut className="h-4 w-4"/><span className="hidden sm:inline">{t("nav.logout","Déconnexion")}</span></button>
        </div>
      </header>
