  ...
  return <div className="flex min-h-screen w-full bg-kelo-background font-sans text-kelo-text">
    ...
    <main className="min-h-screen min-w-0 flex-1 bg-white pb-24 md:border-x md:border-kelo-border md:pb-0">
      ...
      <div className="mx-auto grid w-full max-w-6xl lg:grid-cols-[300px_minmax(0,1fr)] xl:grid-cols-[320px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 bg-white"><div className="border-b border-kelo-border px-4 py-3.5 sm:px-6 md:py-4"><h2 className="text-lg font-bold text-kelo-text">{sectionTitle}</h2></div><div className="settings-bluesky-section min-w-0">{section==="account"&&<><AccountSection/><RobotAccountSection/></>}{section==="identity"&&<IdentitySection/>}{section==="appearance"&&<KeloIdFeatureLock feature="les paramètres d’affichage" mode="block"><DisplaySection/></KeloIdFeatureLock>}{section==="moderation"&&<ModerationSection/>}{section==="privacy"&&<PrivacySection/>}{section==="notifications"&&<NotificationFeedSection/>}{section==="language"&&<LanguageContentSection/>}{section==="messaging"&&<MessagingSection/>}{section==="legal"&&<LegalSection/>}</div></section>
      </div>
    </main>
  </div>
}
