...
export default function PostCard({ post,isMine,isBookmarked,replyOpen,replyText,onToggleReply,onReplyTextChange,onSendReply,onLike,onRepost,onBookmark,onDelete,onBlocked,onMuted,disableThreadLink }:PostCardProps) {
  ...
  useEffect(()=>{const id=window.setInterval(()=>setClock(v=>v+1),60000);return()=>window.clearInterval(id)},[]);
  const relativeTime=useMemo(()=>formatRelativeTime(post.record?.createdAt||post.indexedAt),[post.record?.createdAt,post.indexedAt,clock]);
  ...
  return <article onClick={handleCardClick} className={`p-4 transition-colors hover:bg-kelo-background/60 ${disableThreadLink?"":"cursor-pointer"}`}>
    ...
    <div className="flex gap-3"><Link href={`/profile/${handle}`} onClick={e=>e.stopPropagation()} className="flex-shrink-0"><Avatar src={post.author?.avatar} fallback={handle?handle[0].toUpperCase():"U"}/></Link>
      <div className="min-w-0 flex-grow"><div className="flex flex-wrap items-start justify-between gap-2"><div className="flex min-w-0 flex-wrap items-center gap-2"><Link href={`/profile/${handle}`} onClick={e=>e.stopPropagation()} className="flex min-w-0 flex-wrap items-center gap-2 hover:underline"><span className="max-w-full truncate font-bold text-kelo-primary">{post.author?.displayName||"Utilisateur"}</span></Link><span onClick={e=>e.stopPropagation()} className="flex flex-shrink-0 items-center"><AccountBadges actor={post.author} identitySize="sm" certificationSize={16} gap="xs"/></span>{relativeTime&&<span className="flex-shrink-0 text-sm text-kelo-primary/60">· {relativeTime}</span>}</div>{isMine&&onDelete&&<button type="button" onClick={e=>{e.stopPropagation();onDelete()}} className="flex-shrink-0 text-xs font-bold text-kelo-muted transition-colors hover:text-kelo-danger" title="Supprimer" aria-label="Supprimer la publication">🗑️</button>}</div>
