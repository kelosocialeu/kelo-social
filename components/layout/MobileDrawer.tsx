      ...
      <div className="relative min-h-0 flex-1 overflow-y-auto px-4 py-4">
        ...
        <nav className="flex flex-col gap-1">
          ...
          {checked && isAdmin && <div className="mt-3 rounded-2xl border border-kelo-primary/20 bg-white/55 p-2">
            ...
            <Link href="/admin" prefetch onClick={(event) => navigate(event, "/admin")} className={`flex touch-manipulation items-center gap-4 rounded-xl px-3 py-3 text-sm font-semibold transition ${pathname === "/admin" ? "bg-kelo-gradient text-white shadow-sm" : "text-kelo-secondary hover:bg-white/70"}`}><ShieldCheck className="h-5 w-5" />{t("nav.admin", "Tableau de bord")}</Link>
            <Link href="/admin/certifiers" prefetch onClick={(event) => navigate(event, "/admin/certifiers")} className={`flex touch-manipulation items-center gap-4 rounded-xl px-3 py-3 text-sm font-semibold transition ${isRouteActive(pathname, "/admin/certifiers") ? "bg-kelo-gradient text-white shadow-sm" : "text-kelo-secondary hover:bg-white/70"}`}><BadgeCheck className="h-5 w-5" />{t("nav.certifications", "Certifications")}</Link>
            <Link href="/admin/certifiers" prefetch onClick={(event) => navigate(event, "/admin/certifiers")} className="flex touch-manipulation items-center gap-4 rounded-xl px-3 py-3 text-sm font-semibold text-kelo-secondary transition hover:bg-white/70"><Bot className="h-5 w-5" />{t("nav.robotAccounts", "Comptes robots")}</Link>
          </div>}
        </nav>
      </div>
      <div className="relative border-t border-white/40 px-4 pb-[max(16px,env(safe-area-inset-bottom))] pt-4"><button type="button" onClick={onLogout} className="flex w-full touch-manipulation items-center justify-center gap-2 rounded-2xl bg-white/65 py-3 text-sm font-bold text-kelo-text transition active:scale-[0.98]"><LogOut className="h-4 w-4" />{t("nav.logout", "Déconnexion")}</button></div>
    </aside>
  </>;
}
