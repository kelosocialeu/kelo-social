    ...
    <>
        ...
        <>
          ...
          <div className="fixed inset-x-4 bottom-[calc(92px+env(safe-area-inset-bottom))] z-[60] mx-auto max-w-md rounded-3xl border border-white/50 bg-white/95 p-3 shadow-2xl backdrop-blur-2xl md:hidden">
            <div className="mb-1 flex items-center justify-between px-2 py-1">
              <p className="text-sm font-extrabold text-kelo-text">Créer</p>
              <button
                type="button"
                onClick={() => setCreateMenuOpen(false)}
                className="flex h-8 w-8 items-center justify-center rounded-full bg-kelo-background text-kelo-muted"
                aria-label="Fermer"
              >
                <X className="h-4 w-4" />
              </button>
            ...
            <button
              type="button"
              onClick={startPost}
              className="flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition active:scale-[0.98] hover:bg-kelo-background"
            >
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-kelo-gradient text-white shadow-md">
                <FileText className="h-5 w-5" />
              </span>
              <span>
                <span className="block font-extrabold text-kelo-text">Publication</span>
                <span className="block text-xs text-kelo-muted">Texte, photo, GIF ou vidéo</span>
              </span>
            </button>
            ...
            <button
              type="button"
              onClick={startReel}
              className="mt-1 flex w-full items-center gap-3 rounded-2xl px-3 py-3 text-left transition active:scale-[0.98] hover:bg-kelo-background"
            >
              <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-black text-white shadow-md">
                <Video className="h-5 w-5" />
              </span>
              <span>
                <span className="block font-extrabold text-kelo-text">Réaliser un Réel</span>
                <span className="block text-xs text-kelo-muted">Ouvrir la caméra, filmer puis publier</span>
              </span>
            </button>
          </div>
        </>
      ...
      <nav aria-label={t("nav.mobile", "Navigation mobile")} className={`fixed inset-x-0 bottom-0 z-50 flex justify-center px-3 pb-[max(12px,env(safe-area-inset-bottom))] transition-all duration-300 ease-out md:hidden ${hidden ? "translate-y-[140%] opacity-0" : "translate-y-0 opacity-100"}`}>
        <div className="relative flex h-[68px] w-full max-w-md items-center justify-between rounded-[28px] border border-white/30 bg-white/65 px-3 shadow-[0_18px_55px_rgba(67,24,130,0.28)] backdrop-blur-2xl supports-[backdrop-filter]:bg-white/55">
          ...
          <button type="button" onClick={() => setCreateMenuOpen((value) => !value)} aria-label="Créer" aria-expanded={createMenuOpen} className="relative z-10 -mt-8 flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-full bg-kelo-gradient text-white shadow-[0_12px_32px_rgba(139,92,246,0.45)] transition-all duration-200 hover:scale-105 active:scale-95"><span className="absolute inset-0 rounded-full border border-white/35"/><Plus className={`relative h-7 w-7 transition-transform duration-200 ${createMenuOpen ? "rotate-45" : ""}`} strokeWidth={2.4}/></button>
