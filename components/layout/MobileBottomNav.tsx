...
export default function MobileBottomNav({ handle, hidden = false, onCreatePost }: MobileBottomNavProps) {
  ...
  return (
    <>
      ...
      {createMenuOpen && (
        <>
          <button type="button" aria-label={t("common.close", "Fermer le menu de création")} className="fixed inset-0 z-[58] bg-black/25 backdrop-blur-[2px] md:hidden" onClick={() => setCreateMenuOpen(false)} />
          <div className="fixed inset-x-4 bottom-[calc(92px+env(safe-area-inset-bottom))] z-[60] mx-auto max-w-md rounded-3xl border border-white/50 bg-white/95 p-3 shadow-2xl backdrop-blur-2xl md:hidden">
            <div className="mb-1 flex items-center justify-between px-2 py-1">
              ...
              <button type="button" onClick={() => setCreateMenuOpen(false)} className="flex h-8 w-8 items-center justify-center rounded-full bg-kelo-background text-kelo-muted" aria-label={t("common.close", "Fermer")}><X className="h-4 w-4" /></button>
            </div>
            <button type="button" onClick={startPost} className="flex w-full touch-manipulation items-center gap-3 rounded-2xl px-3 py-3 text-left transition active:scale-[0.98] hover:bg-kelo-background"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-kelo-gradient text-white shadow-md"><FileText className="h-5 w-5" /></span><span><span className="block font-extrabold text-kelo-text">{t("nav.createPost", "Publication")}</span><span className="block text-xs text-kelo-muted">{t("composer.placeholder", "Texte, photo, GIF ou vidéo")}</span></span></button>
            <button type="button" onClick={startReel} className="mt-1 flex w-full touch-manipulation items-center gap-3 rounded-2xl px-3 py-3 text-left transition active:scale-[0.98] hover:bg-kelo-background"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-black text-white shadow-md"><Video className="h-5 w-5" /></span><span><span className="block font-extrabold text-kelo-text">{t("nav.reels", "Réaliser un Réel")}</span><span className="block text-xs text-kelo-muted">{t("nav.reelsDescription", "Ouvrir la caméra, filmer puis publier")}</span></span></button>
          </div>
        </>
      )}
