...
export default function MobileBottomNav({ handle, hidden = false, onCreatePost }: MobileBottomNavProps) {
  ...
  const navItems = [
    ...
    { href: "/search", label: t("nav.explore", "Explorer"), icon: Search },
  ...
  return (
    <nav aria-label={t("nav.mobile", "Navigation mobile")} className={`fixed inset-x-0 bottom-0 z-50 flex justify-center px-2 pb-[max(12px,env(safe-area-inset-bottom))] transition-all duration-300 ease-out md:hidden ${hidden ? "translate-y-[140%] opacity-0" : "translate-y-0 opacity-100"}`}>
      <div className="relative flex h-[68px] w-full max-w-[470px] items-center justify-between rounded-[28px] border border-white/30 bg-white/65 px-2 shadow-[0_18px_55px_rgba(67,24,130,0.28)] backdrop-blur-2xl supports-[backdrop-filter]:bg-white/55">
...
function MobileNavLink({ href, label, icon: Icon, active }: MobileNavLinkProps) {
  return <Link href={href} aria-label={label} aria-current={active ? "page" : undefined} className="relative z-10 flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-2xl transition-all duration-200 active:scale-90 sm:h-12 sm:w-12">
    {active && <span className="absolute inset-1 rounded-2xl bg-kelo-gradient shadow-[0_7px_18px_rgba(139,92,246,0.28)]"/>}
    <Icon className={`relative h-[21px] w-[21px] transition-colors sm:h-[22px] sm:w-[22px] ${active ? "text-white" : "text-kelo-muted"}`} strokeWidth={active ? 2.4 : 2}/>
