...
export default function VerificationBadge({ actor, size = 16 }: VerificationBadgeProps) {
  ...
  const handleClick = async (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    setLoadingIssuer(true); setIssuerError(false);
    try {
      const allRecords = await listCertifications();
      const trustedDids = new Set(allRecords.filter((r) => r.status === "trusted-verifier").map((r) => normalizeDid(r.subjectDid)));
      const entries: Array<{ did?: string; handle?: string; displayName?: string; source: "native" | "kelo" }> = getVerificationIssuers(nativeActor).map((issuer) => ({ did: issuer.issuer, source: "native" }));
      for (const certification of keloCertifications) {
        if (certification.status !== "certified") continue;
        entries.push({ did: certification.issuerDid, handle: certification.issuerHandle || (!certification.issuerDid ? "kelosocial.eu" : undefined), displayName: !certification.issuerDid ? "Kelo Social" : undefined, source: "kelo" });
      }
      const unique = entries.filter((entry, index, all) => {
        const key = entry.did?.toLowerCase() || entry.handle?.toLowerCase();
        return !!key && all.findIndex((candidate) => (candidate.did?.toLowerCase() || candidate.handle?.toLowerCase()) === key) === index;
      });
      const profiles = await Promise.all(unique.map(async (entry) => {
        if (entry.did) {
          try {
            const profile = await getIssuerProfile(entry.did);
            const profileBadge = getVerificationBadge(profile);
            return { did: entry.did, handle: profile.handle, displayName: profile.displayName, avatar: profile.avatar, source: entry.source, trusted: profileBadge === "trusted-verifier" || trustedDids.has(normalizeDid(entry.did)) } as IssuerProfile;
          } catch {}
        }
        if (entry.handle) return { handle: entry.handle, displayName: entry.displayName, source: entry.source, trusted: entry.handle.toLowerCase() === "kelosocial.eu" } as IssuerProfile;
        return null;
      }));
      const valid = profiles.filter((profile): profile is IssuerProfile => Boolean(profile?.handle));
      setIssuers(valid); setIssuerError(valid.length === 0);
    } catch { setIssuerError(true); } finally { setLoadingIssuer(false); }
  };
  ...
  return <>
    ...
    {open && <><div className="fixed inset-0 z-30 bg-black/10" onClick={() => setOpen(false)} />
      {badgeType === "verified" ? <div className="fixed left-1/2 top-1/2 z-40 max-h-[80vh] w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-2xl border border-kelo-border bg-white p-5 shadow-kelo" onClick={(e) => e.stopPropagation()}>
        ...
        {!loadingIssuer && issuers.length > 0 && <><p className="mb-2 text-xs text-kelo-muted">{issuers.length > 1 ? `Certifié par ${issuers.length} certificateurs de confiance :` : "Certifié par :"}</p><div className="space-y-1">{issuers.map((issuer) => <Link key={issuer.did || issuer.handle} href={`/profile/${issuer.handle}`} className="flex items-center gap-3 rounded-xl p-2 hover:bg-kelo-background" onClick={() => setOpen(false)}><Avatar src={issuer.avatar} fallback={issuer.handle[0]?.toUpperCase() || "K"} size="sm" /><div className="min-w-0 flex-1"><div className="flex min-w-0 items-center gap-1.5"><p className="truncate text-sm font-bold text-kelo-text">{issuer.displayName || issuer.handle}</p>{issuer.trusted && <Badge status="trusted-verifier" size={16} />}</div><p className="truncate text-xs text-kelo-muted">@{issuer.handle}{issuer.source === "kelo" ? " · Kelo" : " · AT Protocol"}</p></div></Link>)}</div></>}
      ...
      </div> : <div className="fixed left-1/2 top-1/2 z-40 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-kelo-border bg-white p-6 shadow-kelo" onClick={(e) => e.stopPropagation()}><div className="mb-5 rounded-2xl bg-kelo-background p-5"><div className="flex items-center justify-center gap-3"><div className="flex flex-col items-center gap-2"><div className="flex h-14 w-14 items-center justify-center rounded-full bg-kelo-gradient"><Logo className="h-8 w-8" /></div><span className="text-xs font-semibold text-kelo-muted">Kelo</span></div><ArrowRight className="h-4 w-4 text-kelo-muted" /><img src={CERTIFIER_IMAGE} alt="Certificateur de confiance" className="h-14 w-14 object-contain" /><ArrowRight className="h-4 w-4 text-kelo-muted" /><img src={VERIFIED_IMAGE} alt="Compte certifié" className="h-14 w-14 object-contain" /></div></div><h3 className="mb-2 text-center text-base font-extrabold text-kelo-text">{actor?.displayName || actor?.handle} est un certificateur de confiance</h3><p className="mb-5 text-center text-sm text-kelo-muted">Ce statut peut provenir de Kelo Social ou d’un service AT Protocol synchronisé.</p><button type="button" onClick={() => setOpen(false)} className="w-full rounded-full bg-kelo-background py-2.5 text-sm font-bold text-kelo-text">Fermer</button></div>}
    </>}
  </>;
}
