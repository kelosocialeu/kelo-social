...
const KELO_SOCIAL_CALLBACK_ORIGIN = "https://kelosocial.eu";
...
export default function KeloIdMobileLoginButton() {
  ...
  async function continueWithKeloId() {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || "Connexion Kelo ID impossible."
        );
      }
      ...
      // Utilise toujours l'origine canonique acceptée par Kelo ID.
      // Cela évite le problème www.kelosocial.eu vs kelosocial.eu
      // dans la WebView Android.
      const callback = new URL(
        "/kelo-id/callback",
        KELO_SOCIAL_CALLBACK_ORIGIN
      );
  ...
  return (
    <section className="mb-5 border-b border-kelo-border pb-5">
      ...
      >
        {loading
          ? "Ouverture de Kelo ID..."
          : "Continuer avec Kelo ID"}
      </button>
      ...
      {error && (
        <p
          role="alert"
          className="mt-2 text-center text-sm font-medium text-kelo-danger"
        >
          {error}
        </p>
      )}
