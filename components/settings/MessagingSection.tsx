  ...
  return (
    <section className="rounded-2xl border border-kelo-border bg-white p-4 sm:p-5">
      <div className="flex items-start gap-3">
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-kelo-background">
          <Icon className="h-5 w-5" />
        </span>
        <div>
          <h3 className="font-extrabold text-kelo-text">{title}</h3>
          <p className="mt-1 text-sm leading-5 text-kelo-muted">{description}</p>
        </div>
      </div>
        ...
        {OPTIONS.map((option) => (
          <label
            ...
            className={`flex cursor-pointer items-start gap-3 rounded-xl border p-3 transition ${
              value === option.value
                ? "border-kelo-primary bg-kelo-primary/5"
          ...
          >
            <input
              ...
              disabled={disabled}
              className="mt-1 h-4 w-4 accent-kelo-primary"
            />
            <span>
              <span className="block text-sm font-bold text-kelo-text">{option.label}</span>
              <span className="mt-0.5 block text-xs text-kelo-muted">{option.description}</span>
            </span>
          </label>
        ))}
      </div>
...
export default function MessagingSection() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err.message : "Impossible de charger les paramètres de messagerie.");
        }
      } finally {
  ...
  const save = async (nextIncoming: ChatPermission, nextGroupInvites: ChatPermission) => {
    ...
    setMessage("");
    try {
      await setMessagingPreferences({
        allowIncoming: nextIncoming,
        allowGroupInvites: nextGroupInvites,
      });
      setMessage("Paramètres synchronisés avec AT Protocol.");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Impossible d’enregistrer ces paramètres.");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return <div className="p-6 text-sm text-kelo-muted">Chargement des paramètres de messagerie…</div>;
  }

  return (
    <div className="space-y-4 p-4 sm:p-6">
      <PreferenceBlock
        title="Autoriser les messages privés de"
        description="Choisissez qui peut démarrer une nouvelle conversation privée avec vous."
      ...
      <PreferenceBlock
        title="Autoriser les invitations de groupe de"
        description="Choisissez qui peut vous inviter dans une conversation de groupe."
        icon={UsersRound}
        value={allowGroupInvites}
        onChange={changeGroupInvites}
        disabled={saving}
      />
      ...
      <p className="text-xs leading-5 text-kelo-muted">
        Ces choix sont enregistrés dans votre déclaration de messagerie AT Protocol et peuvent être reconnus par les applications compatibles.
      </p>
    </div>
  );
}
