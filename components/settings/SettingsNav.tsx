...
const GROUPS: Array<{ title: string; items: SettingsItem[] }> = [
  ...
  },
  {
    ...
    items: [
      { key: "moderation", label: "Modération", description: "Comptes bloqués, masqués et contenu sensible", icon: Eye },
    ],
  },
];
...
export default function SettingsNav({ active, onChange }: SettingsNavProps) {
  return (
    <>
      <nav aria-label="Sections des paramètres" className="border-b border-kelo-border bg-white lg:hidden">
        <div className="p-3 sm:p-4">
          <p className="mb-3 text-xs font-bold uppercase tracking-wide text-kelo-muted">
            Tous les paramètres
          </p>
            ...
            {COMPACT_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex min-h-[78px] min-w-0 flex-col items-start justify-between gap-2 rounded-2xl border p-3 text-left transition ${
                    selected
                ...
                >
                  ...
                  <span className="w-full break-words text-xs font-extrabold leading-4 sm:text-sm">
                    {item.label}
                  </span>
                </button>
              );
      ...
      <nav aria-label="Sections des paramètres" className="hidden w-full bg-white lg:block">
        {GROUPS.map((group) => (
          <section key={group.title} className="border-b border-kelo-border py-3 last:border-b-0">
              ...
              {group.items.map((item) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => onChange(item.key)}
                    className={`flex w-full items-center gap-3 px-4 py-3.5 text-left transition-colors xl:px-5 ${selected ? "bg-kelo-background" : "hover:bg-kelo-background/70"}`}
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block text-[15px] font-semibold text-kelo-text">{item.label}</span>
                      <span className="mt-0.5 block text-xs leading-5 text-kelo-muted">{item.description}</span>
                    </span>
        ...
        <div className="border-t border-kelo-border px-4 py-4 text-xs text-kelo-muted xl:px-5">
          <div className="flex items-start gap-2">
            ...
            <span>Les réglages sont enregistrés sur cet appareil ou via AT Protocol selon l’option.</span>
          </div>
        </div>
      </nav>
    </>
