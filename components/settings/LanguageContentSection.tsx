...
export default function LanguageContentSection() {
  ...
  const toggle = (field: "postLanguages" | "interests", value: string) =>
    save({
      ...prefs,
      [field]: prefs[field].includes(value)
        ? prefs[field].filter(v => v !== value)
        : [...prefs[field], value],
    });
  ...
  return <div className="flex flex-col gap-8 p-4 sm:p-6">
    <section>
      <div className="flex items-center gap-2">
        ...
        <h3 className="font-extrabold">{t("settings.language.title", "Langue de Kelo Social")}</h3>
      </div>
      ...
      <p className="mt-2 text-xs text-kelo-muted">
        {t("settings.language.staticNotice", "Les langues affichées ici disposent d’un fichier d’interface statique installé sur Kelo Social.")}
      </p>
    </section>

    <section className="border-t border-kelo-border pt-6">
      <div className="flex items-center gap-2">
        <Languages className="h-5 w-5 text-kelo-primary"/>
        <h3 className="font-extrabold">{t("settings.postLanguages.title", "Langues des publications")}</h3>
      </div>
      ...
      <div className="mt-4 flex max-h-72 flex-wrap gap-2 overflow-y-auto rounded-2xl border border-kelo-border p-3">
        {KELO_LANGUAGES.filter(([c]) => c !== "auto").map(([code, label]) => (
          <button
            key={code}
            type="button"
            onClick={() => toggle("postLanguages", code)}
            className={`rounded-full border px-3 py-2 text-xs font-bold ${prefs.postLanguages.includes(code) ? "border-kelo-primary bg-kelo-primary/10 text-kelo-primary" : "border-kelo-border"}`}
          >
            {label}
          </button>
        ))}
    ...
    <section className="border-t border-kelo-border pt-6">
      <div className="flex items-center gap-2">
        <Sparkles className="h-5 w-5 text-kelo-primary"/>
        <h3 className="font-extrabold">{t("settings.interests.title", "Centres d’intérêt")}</h3>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {KELO_INTERESTS.map(item => (
          <button
            key={item}
            type="button"
            onClick={() => toggle("interests", item)}
            className={`rounded-full border px-3 py-2 text-xs font-bold ${prefs.interests.includes(item) ? "border-kelo-primary bg-kelo-primary/10 text-kelo-primary" : "border-kelo-border"}`}
          >
            {item}
          </button>
        ))}
